# code from https://github.com/petals-infra/health.petals.dev
# to be able to display the online servers inside the chat page
# Also simulates the existence of a configured lightning address 
# for each server, information that is later used to split the
# user's payment
import datetime
import time
import threading
...
class StateUpdaterThread(threading.Thread):
    ...
    def update(self) -> None:
        lightning_addresses = [f"server{i}@analystr.xyz" for i in range(1, 10)]
        bootstrap_peer_ids = []
        ...
        for model in config.MODELS:
            ...
            for peer_id, server in sorted(model_servers):
                lightning_address = lightning_addresses[current_peer_index % len(lightning_addresses)]
                ...
                peer_data[row["short_peer_id"]] = lightning_address

            report = asdict(model)
            report.update({"state": model_state, "server_rows": server_rows})
            model_reports.append(report)
            self.app.config["peer_data"] = peer_data
